...
static complex<double> poly(const vec &p) {
  ...
  const double y = p.y() + 0.;
  return (x - 1) * (x + 1) * (y - 1) * (y + 1);
}

static complex<double> poly_xx(const vec &p) {
...
static complex<double> poly_yy(const vec &p) {

  const double x = p.x() + 0.;
  const double y = p.y() + 0.;
  // master_printf("x %e, y %e\n",x, y);
  return (2) * (x - 1) * (x + 1);
}
...
void getJ(fields &f, complex<realnum> *dvector) {

  f.zero_fields();
  f.initialize_field(Dz, Jz_poly);
  D_to_array(f, dvector);
  f.zero_fields();
}
...
static void add_cw_source(fields &f, const std::string &mode, double omega_src, double scale) {
  ...
  if (mode == "cw") {
    ...
    f.add_volume_source(Dz, src, f.v, Jz_poly, std::complex<double>(0, 1));
  }
  else if (mode == "sin") {
    ...
    custom_src_time src(sin_current, NULL, -meep::infinity,
                        meep::infinity); //,
                                         // omega_src); // / (2 * PI));
    src.is_integrated = false;
    f.add_volume_source(Dz, src, f.v, Jz_poly, scale);
  }
  else
...
void pi_steps_old(const double omega, const double Tend, const size_t Nt, fields &f,
  ...
  realnum *bxy_sol_raw = new realnum[2 * N_D];
  complex<realnum> *b_sol_vector = reinterpret_cast<complex<realnum> *>(bxy_sol_raw);
  ...
  for (int iwh = 0; iwh < N_iwh; iwh++) {
    ...
    bfilt1 = std::cos(t * omega) - 0.25;
    bfilt2 = std::cos((t - f.dt) * omega) - 0.25;
    for (size_t i = 0; i < N_B; i++) {
      bxy_wh_vector[i] = bxy_vector[i] * (bfilt1 + bfilt2) * 0.5;
    }
    ...
    // Move to single for-loop
    rD2 = 0.0;
    for (size_t i = 0; i < N_D; i++) {
      dz_wh_vector[i] *= f.dt * 2 / Tend;
    ...
    master_printf("Iter: %5d, error D: %2e,  D residaul norm: %2e B residaul norm: %2e \n", iwh,
                  std::sqrt(eD), std::sqrt(rD2), std::sqrt(rB2));
...
void pi_steps(const double omega, const double Tend, const size_t Nt, fields &f,
              complex<realnum> *dz_wh_vector, size_t N_D, complex<realnum> *bxy_wh_vector,
              size_t N_B) {
  ...
  for (int iwh = 0; iwh < N_iwh; iwh++) {
    ...
    for (size_t it = 1; it <= Nt; it++) {
      ...
      D_to_array(f, dz_vector);
      // for (int i = 0; i < N_D; i++) {
      //	dz_vector[i] += std::sin((t-f.dt) * omega)*Jvec[i];
      //}
      // array_to_D(dz_vector, f);
      B_to_array(f, bxy_vector); // Originally did the integration in-place.
...
int main(int argc, char **argv) {
  ...
  pi_steps(omega, Tend, Nt, f, d_wh_vector, N_D, b_wh_vector, N_B);
